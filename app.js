...
safe.run(function(cb) {
	safe.run(function(cb) {
		...
		var staticPathMin = __dirname + '/modules/static_min/';
		fs.mkdir(staticPathMin, function(err) {
			if (err) {
				staticPath = staticPathMin;
				return cb(null);
			}
			
			api.core.minify(staticPath, safe.sure_result(cb, function(files) {
				staticPath = staticPathMin;
			}));
		});
	...
	console.log('Run server on port: '.blue + port.toString().yellow);
	app.listen(port);
})
